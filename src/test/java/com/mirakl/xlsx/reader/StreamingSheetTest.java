...
import com.mirakl.xlsx.reader.impl.XlsxHyperlink;
...
public class StreamingSheetTest {
  ...
  public void testLastRowNum() throws Exception {
    try(
            InputStream is = TestUtils.getInputStream("large.xlsx");
    ...
    try(
            InputStream is = TestUtils.getInputStream("empty_sheet.xlsx");
            Workbook workbook = StreamingReader.builder().open(is);
    ) {
      assertEquals(1, workbook.getNumberOfSheets());
      Sheet sheet = workbook.getSheetAt(0);
  ...
  public void testHyperlinksEnabled() throws Exception {
    try (
            InputStream is = TestUtils.getInputStream("59775.xlsx");
      ...
      while(rowIterator.hasNext()) {
        Row row = TestUtils.nextRow(rowIterator);
        if (row.getRowNum() == 1) {
          a2 = row.getCell(0);
  ...
  public void testSharedHyperlink() throws Exception {
    try (
            InputStream is = TestUtils.getInputStream("sharedhyperlink.xlsx");
            Workbook workbook = StreamingReader.builder().setReadHyperlinks(true).open(is);
    ) {
      Sheet sheet = workbook.getSheetAt(0);
      Iterator<Row> rowIterator = sheet.rowIterator();
  ...
  public void testGetActiveCell() throws Exception {
    try (
      ...
      Sheet sheet = workbook.getSheetAt(0);
      TestUtils.nextRow(sheet.rowIterator()); //need to force a read of first row before getActiveCell works
